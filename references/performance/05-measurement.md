...
Call `initRUM()` once at app boot. `reportAllChanges: false` is the default — reports the final session value, not intermediate updates.
...
1. Group INP events by `attribution.interactionType` — is it scroll, click, keypress, or pointer?
2. Group by `attribution.interactionTarget` — is one component disproportionately slow?
3. For the top offenders, inspect `attribution.longAnimationFrameEntries` — which script?
