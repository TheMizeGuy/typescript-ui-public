...
When a polymorphic component would change behavior with the tag (focus management, keyboard nav, ARIA defaults), **stop and split** — `<LinkButton>` and `<Button>` are clearer than one `<Button as="a">`.
